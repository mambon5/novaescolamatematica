  <?php 
    $vista="profes";
    include("../php/connexion_base.php");//contains basic connection settings
    include("../php/get_elems.php");//contains basic connection settings
    
    // aquesta variable farà que depen dels que visitin la pagina, puguin canviar
    //el contingut (no capats) o no (sí capats)
    $capat = "si";
    ...
    if(isset($_GET['mostra']) && $_GET['mostra']=="tot"){
       //shows everything
    } else {
        //assumim $mostra només 1 persona, o no està posat
        //assumim $veuere te el acronim duna persona.Shows partial database
         if(acronim($vista)) {
             $array = json_decode($persones);
             $key = array_search($vista, array_column($array, 'acronim'));
             $persones = json_encode([$array[$key]]);//add the outer [] to make it
                                                   //an array
             $profes = $persones;
             $alumnes = $persones;
         } else {
...
$conn->close();//close the php-server connection
 ?>
...
<html>
    <head>
       <link rel="icon" type="image/x-icon" href="/w3images/favicon.png">
        ...
        <head>
    <title>Veient les <?php  echo  $vista ?></title>
    ...
    <script src="/js/browsers_utils.js"></script>
...
<!--     <script type="text/javascript" src="js/profes.json"></script>-->
    ...
    <script>
      ...
      profes = JSON.parse(profes)
      
      var alumnes=<?php  echo "'" . $alumnes . "'" ?>;
      alumnes = JSON.parse(alumnes)
      
      var persones=<?php  echo "'" . $persones . "'" ?>;
      persones = JSON.parse(persones)
      
      var vista=<?php  echo "'".$vista."'" ?>;
      var capat=<?php  echo "'".$capat."'" ?>;
      
      if(vista == "profes") {
          people = profes;
      } else if(vista == "alumnes") {
          people = alumnes;
      } else {
          people = persones;
      }
...
//        json_to_select(valclaus,"nom","clau","valclaus")
    ...
    <body onload="">
        ...
        <div class="w3-bar w3-teal w3-top" onclick="history_back()">
       ...
       <h3 class="w3-center w3-text-black">
                  ...
                  <?php 
                  if($vista=="alumnes" ||$vista=="profes") {
                      echo  "Veient les " . $vista; 
                  } else {
                      echo  "Veient " . $vista; 
                  }
                              ?> 
...
<table id="taula" 
    class="w3-table w3-center-all w3-text-black w3-bordered w3-hoverable-gray" >
                ...
                <form id="formdispo" action="" style="display:none"  method="post">form
                    <input id="acronim" name="acronim" type="text">
                     <input id="tipureg" name="tipureg" type="text" value="veure">
                     <input id="vista" name="vista" type="text" value="veure">
                     <input id="camps" name="camps" type="text" value="tots">
                     <input id="capat" name="capat" type="text" value="no">

                </form>
                <form id="formclases" action="" style="display:none"  method="post">form
                    <input id="acronim" name="acronim" type="text">
                </form>
    ...
    <script>
        ...
        if(persones.length>0) {
            var columns =  Object.getOwnPropertyNames(profes[0]);
            print_table(people, columns, taula.id)
            addcolumn_hc(taula)
            addcolumn_regclase(taula)
            hidecolum(taula, 1)
            if(capat != "no") { 
                hidecolum(taula, 2)
                hidecolum(taula, 4)
