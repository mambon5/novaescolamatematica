  <?php 
  
      include("../php/connexion_base.php");//contains basic connection settings
      include("../php/get_elems.php");//contains basic connection settings

      //per veure totes les calsses posar la variable mostra=tot.
  
      // s'ha de posar mostra=parcial per poder canviar les classes.
      $mostra="parcial";
      $vista="total";
      $camps="parcial";
      $tipureg="veure";

     if(isset($_GET['mostra']) && $_GET['mostra'] == "tot"){
         //si mostra=tot a la url de la pagina, es mostren i es poden modificar
         //totes les classes
      $mostra = $_GET['mostra'];
      $vista="total";
      $camps="tots";
        
     } elseif(isset($_POST['vista'])) {
        $vista = $_POST['vista']; 
        $camps = $_POST['camps']; 
        $mostra="parcial";
       
            $arr=array("id_clase","dia", "temps", "desplaçament", "cafe", "nom_profe", "nom_alumne", 
            "comentari");
      
      if($camps=="tots") {         
            $arr =  array_merge($arr, array("pagada_by_client", "pagada_to_profe"));
      }    

        if(acronim($vista)) {
             $clases =   get_clases_4person($arr, $vista);
        }
    } else {
        $clases = [];
    }
    
    $conn->close();//close the php-server connection
 ?>
     
    
     

<!DOCTYPE html>
<!--img data url for logo <img src="data:image/png;base64," alt="" />-->
<!--
To change this license header, choose License Headers in Project Properties.
To change this template file, choose Tools | Templates
and open the template in the editor.
-->
<html>
    <head>
       <link rel="icon" type="image/x-icon" href="/w3images/favicon.png">

        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="/w3css/4/w3.css">
        <link rel="stylesheet" href="/lib/w3-theme-black.css">

        
        <link rel="stylesheet" href="/w3css/4/fonts.css">
        <head>
    <title>Veient les matèries</title>
   <link rel="stylesheet" href="/css/slideformw3.css">
    <!--<link rel="stylesheet" href="css/calform.css">-->
    <style>
        body {font-family: Pier}
    </style>
   
    
    <!-- for JQuery date picker -->
    <script src="/js/JQuery/jquery-3.6.0.min.js"></script>
    <script src="/js/JQuery/jquery-ui-1.13.0.custom/jquery-ui.js"></script>
    <link rel="stylesheet" href="/js/JQuery/jquery-ui-1.13.0.custom/jquery-ui.css">
    <link rel="stylesheet" href="/js/JQuery/jquery-ui-1.13.0.custom/jquery-ui.theme.css">
    
    <script src="/js/browsers_utils.js"></script>
    <script src="/js/list_json.js"></script>
    <script src="/js/print_table.js"></script>
    <script src="/js/find_property.js"></script>
    <script src="/js/matrix_ops.js"></script>
    
<!--     <script type="text/javascript" src="js/profes.json"></script>-->
    
    
    
    <script>
      var vista=<?php  echo "'".$vista."'" ?>;
      clases=<?php  echo $clases ?>;
      var profes=<?php echo "'".$profes."'" ?>; 
      persones=<?php echo "'".$persones."'" ?>;
      var mostra=<?php echo "'".$mostra."'" ?>;
      var tipureg=<?php echo "'".$tipureg."'" ?>;
      persones  =  JSON.parse(persones)
    profes  =  JSON.parse(profes)
    var alumnes=<?php  echo "'".$alumnes."'" ?>; 
    alumnes  =  JSON.parse(alumnes)
  
    selected = false; //selected row in table
    
    function recupera_ultim_fila(row){
         arr = [4,8,9]
         for( var x=0; x<arr.length; x++) {
             j = arr[x]
             elem = row.childNodes[j]
             
             if(elem.childNodes[0].value==1)  elem.innerHTML = "Sí";
             else  elem.innerHTML = "No";
         }
         
         arr = [2,3]
         for( var x=0; x<arr.length; x++) {
             j = arr[x]
             elem = row.childNodes[j]
             elem.innerHTML = elem.childNodes[0].value
             
         }
         arr = [1,7]
         for( var x=0; x<arr.length; x++) {
             j = arr[x]
             elem = row.childNodes[j]
             elem.innerHTML = elem.childNodes[0].value
             
         }
    }
    
    function celes_editables(item) {
        n = item.rowIndex-1
        
        var w = item.childNodes.length;
        
        //mostra opcions per a temps i despl i deixa sel·leccionada la per defecte
        tempo = document.createElement("select");
        tempo.id = "row_temps"
        despli = document.createElement("select");
        despli.id="row_desplç";
        tempinhtml = claserows[n].childNodes[2].innerHTML;
        desplinhtml = claserows[n].childNodes[3].innerHTML;
                
        claserows[n].childNodes[2].innerHTML = "";
        claserows[n].childNodes[3].innerHTML = "";
        claserows[n].childNodes[2].appendChild(tempo);
        claserows[n].childNodes[3].appendChild(despli);
        
        horesdeclase(tempo);tempsdespl(despli);//this fills the select with the time options
        //the following lines select the current value in the select
        claserows[n].childNodes[2].childNodes[0].value=canviaformat(tempinhtml);
        claserows[n].childNodes[3].childNodes[0].value=canviaformat(desplinhtml);

        for(var j=0;j<w;j++) {
            //canvia la cela a editable, si no es el id, el profe ni l'alumne
            
            if(j==1) {//afegint date input per canviar data
                input = document.createElement("input");
                input.type = "date"
                input.value=claserows[n].childNodes[j].innerText
                //next line makes each row update form upon modification of its contents
                input.addEventListener("change", function(){ update_form_canvia_clase() })
//                input.onchange=function(){update_form_canvia_clase()};
                claserows[n].childNodes[j].innerHTML="";
                claserows[n].childNodes[j].appendChild(input);
            }
            
            if(j==7) {//afeging text input per canviar comentari
                input = document.createElement("input");
                input.type = "text"
                input.value=claserows[n].childNodes[j].innerText
                //next line makes each row update form upon modification of its contents
                input.addEventListener("keypress", function(){ update_form_canvia_clase() })
//                input.onchange=function(){update_form_canvia_clase()};
                claserows[n].childNodes[j].innerHTML="";
                claserows[n].childNodes[j].appendChild(input);
                
            
            } 
            if(j==9 || j==4 || j==8) {//afegint select del cafe, pago client
                //i pago profe
                inner="<select>"
                if(claserows[n].childNodes[j].innerHTML!="No") {
                    inner=inner + 
"<option value='1'>Sí</option>\n\
<option value='0'>No</option>"
                } else {
                   inner=inner + 
"<option value='0'>No</option>\n\
<option value='1'>Sí</option>"
    }
                inner = inner + "</select>"
                
                claserows[n].childNodes[j].innerHTML = inner;
                        
            }
        }
        
        //restaurar valors de lultima row seleccionada i no modificada:
        if(selected != false && selected != item) {
            recupera_ultim_fila(selected);
        }
        
        
    }
    
    function update_form_canvia_clase() {//udpate form with the row selected values
            var n = selected.rowIndex-1
            
            id_clase.value = clases[n].id_clase
            dia.value=selected.childNodes[1].childNodes[0].value
            temps.value = selected.childNodes[2].childNodes[0].value
            despl.value = selected.childNodes[3].childNodes[0].value
            cafe.value =  selected.childNodes[4].childNodes[0].value
            comentari.value =  selected.childNodes[7].childNodes[0].value
            pagada_by_client.value =selected.childNodes[8].childNodes[0].value
            pagada_to_profe.value = selected.childNodes[9].childNodes[0].value
            
            alumne.value = clases[n].nom_alumne
            profe.value = clases[n].nom_profe
    }
    
    
    function canvia_clase(item) {
        n = item.rowIndex-1
        set_original_clase(n);
        
        last = table.rows[1].childNodes.length -1 //get number of last column
        hide_columns_in_table(table, [last-1,last])//hide last two for all rows
        
         //to avoid reseting everything if we are clicking onthe same selected row again
        if(selected != item) content_uneditable(table);
         
        
            
            if(n>-1) { //display the "elimina" and "modifica" buttons
                var lastc = claserows[0].childNodes.length -1;
               claserows[n].childNodes[lastc].style.display="table-cell";
               claserows[n].childNodes[lastc-1].style.display="table-cell";
               
            }
            
           
            
            if(selected != item) celes_editables(item);
                    
            
            selected = claserows[n]; //selected row in table
            update_form_canvia_clase();
            
             
      }
      
      function elimina_clase() {
           
          if(confirm("Esteu segura que voleu eliminar la classe?"))  {
              newtipusreg.value = "elimina"; 
              form.submit();
          }
          
          
      }
      row="";
      function modifica_clase(elem) {
        if(confirm("Esteu segura que voleu modificar la classe?"))  {
            newtipusreg.value = "modifica"; 
            update_form_canvia_clase();

            row_clicada= elem.parentNode.parentNode;

            var n = row_clicada.rowIndex-1

            if(id_clase.value != selected.childNodes[0].innerText || selected != row_clicada) {
                console.log("error: fila clicada i sel·leccionada no coincideixen!")
                return;
            } else console.log("es pot modificar la fila: " +(n+1))

            form.submit();
        }

      }
      
      function set_original_clase(n) {
             original_dia.value = clases[n].dia
             original_temps.value = clases[n].temps
             original_despl.value = clases[n].desplaçament
             original_profe.value = clases[n].nom_profe
             original_comentari.value = clases[n].comentari
             original_alumne.value = clases[n].nom_alumne
            original_desplaç.value = clases[n].desplaçament
            original_cafe .value = clases[n].cafe
            
      } 
      
      
        
//        json_to_select(valclaus,"nom","clau","valclaus")
</script>

    </head>
    <body onload="">
        
        <?php if($camps=="tots")
            
            echo '
                <br><br>
        <form id="regForm" style="display:none" action="../php/registres/enviarclase.php" method="post" onsubmit=""
     >
    <input  id="id_clase" placeholder="2"  readonly
               name="id_clase" type="text" required >        

  <input  id="dia" placeholder="yyyy-mm-dd"   readonly
               name="dia" type="date" required >
  <input  id="temps" placeholder="0"   readonly
               name="temps" type="text" required >
  <input  id="despl" placeholder="0"   readonly
               name="tdesp" type="text" required >
  <input  id="alumne" placeholder=""  
               name="nomalumnes" type="text" required  
               readonly>
  <input  id="profe" placeholder=""  
               name="nomprofes" type="text" required readonly>
  <input  id="comentari" placeholder=""  
               name="comentari" type="text" required readonly>
  
  <input  id="cafe" placeholder=""  
               name="cafe" type="text" required readonly>
  <input  id="pagada_by_client" placeholder=""  
               name="pagada_by_client" type="text" required readonly>
  <input  id="pagada_to_profe" placeholder=""  
               name="pagada_to_profe" type="text" required readonly>
  
  <input  id="tipusreg" placeholder=""  
               name="tipureg" type="text" required readonly>
               
      
<div>Dades originals de la classe:</div>
  <input  id="original_dia" placeholder="yyyy-mm-dd"   readonly
               name="original_dia" type="date" required >
  <input  id="original_temps" placeholder="0"   readonly
               name="original_temps" type="text" required >
  <input  id="original_despl" placeholder="0"   readonly
               name="original_tdesp" type="text" required >
  <input  id="original_alumne" placeholder=""  
               name="original_alumne" type="text" required  
               readonly>
  <input  id="original_profe" placeholder=""  
               name="original_profe" type="text" required readonly>
   <input  id="original_cafe" placeholder=""  
               name="original_cafe" type="text" required readonly>
    <input  id="original_desplaç" placeholder=""  
               name="original_desplaç" type="text" required readonly>
  <input  id="original_comentari" placeholder=""  
               name="original_comentari" type="text" required readonly>
  
  

            
        </form>';
            ?>
        
        
    
            
       
        
        <div class="w3-bar w3-teal w3-top" onclick="history_back()">
            <a href="" class="w3-bar-item w3-button">Torna enrere</a>
        </div>
       
       <h3 class="w3-center w3-text-black">
                  <i class="fa fa-home w3-margin-right" ></i>
                  Veient les clases <b>NEM</b></h3>

  
        
      
     <div class="" ><p></p></div>
         
     <div class="recull_taula">
<table id="taula" style="width:90%"
    class="w3-table w3-center-all w3-text-black  w3-bordered w3-hoverable-gray" >

    
</table>
     </div>       
                
        <div class=" " ><p></p></div>  


      
      
    
    </body>
    
    <script src="/js/slidformw3.js"></script>
    <script src="/js/canvia_dispo.js"></script>
    
    <script>
        
        var table = document.getElementById("taula");
        
         <?php if($camps=="tots")            
            echo '
        var id_clase= document.getElementById("id_clase");
        var alumne = document.getElementById("alumne");
        var profe = document.getElementById("profe");
        var dia = document.getElementById("dia");
        var temps = document.getElementById("temps");
        var despl = document.getElementById("despl");
        var cafe = document.getElementById("cafe");
        var pagada_by_client = document.getElementById("pagada_by_client");
        var pagada_to_profe = document.getElementById("pagada_to_profe");
        var comentari = document.getElementById("comentari");
        
        var original_dia = document.getElementById("original_dia");
        var original_temps = document.getElementById("original_temps");
        var original_despl = document.getElementById("original_despl");
        var original_profe = document.getElementById("original_profe");
        var original_comentari = document.getElementById("original_comentari");
        var original_alumne = document.getElementById("original_alumne");
        var original_desplaç = document.getElementById("original_desplaç");
        var original_cafe = document.getElementById("original_cafe");

        
        var form = document.getElementById("regForm");
        ';
            ?>
        var newtipusreg = document.getElementById("tipusreg");
                
        print_clases(clases, persones, "taula");
        hidecolum(taula, 0) //hiding id_clase
        
        var elimclase = "<button onclick='elimina_clase()' class='small_button vividcolor'> Elimina clase </button>"
        var modiclase = "<button onclick='modifica_clase(this)' class='small_button'> Modifica clase </button>"
        
        
       //adds clickable things
       if(mostra=="tot") {
         claserows = document.getElementsByClassName("tabitem");
        //next lines adds "delete" and "modify" clase elements 
        table_add_clickables(claserows, elimclase, modiclase);
        
        
          }     
    </script>



<script src="/js/demanar_cita.js"></script>

   
         <script type="text/javascript" src="/js/profes.js"></script>

    
</html>

