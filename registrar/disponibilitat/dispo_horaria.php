<?php

$tipusreg="nou";

// aquesta variable farà que depen dels que visitin la pagina, puguin canviar
    //el contingut (no capats) o no (sí capats)
    $capat = "si";
    $tipusreg = "veure";
    
    if(isset($_POST['capat'])){
        $capat = $_POST['capat'];
     } 

if(isset($_POST['tipureg'])) {
    $tipusreg = $_POST['tipureg'];
}
include($_SERVER['DOCUMENT_ROOT']."/php/connexion_base.php");//contains basic connection settings
include($_SERVER['DOCUMENT_ROOT']."/php/get_elems.php");//contains basic connection settings
//using the document root server variable, since this folder is inside another folder and "../php" 
// wouldn't work.

$conn->close();//close the php-server connection
?>
<!DOCTYPE html>
<!--img data url for logo <img src="data:image/png;base64," alt="" />-->
<!--
To change this license header, choose License Headers in Project Properties.
To change this template file, choose Tools | Templates
and open the template in the editor.
-->
<html>
    <head>
       <link rel="icon" type="image/x-icon" href="/w3images/favicon.png">

        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="/w3css/4/w3.css">
        <link rel="stylesheet" href="/lib/w3-theme-black.css">

        <!--<link rel="stylesheet" href="/w3css/font-awesome.min.css">-->
        <link rel="stylesheet" href="/w3css/4/fonts.css">
        <head>
    <title>Disponibilitat horaria</title>
    <!--<link rel="stylesheet" href="css/calform.css">-->
    <style>
        body {font-family: Pier}
    </style>
   
    
   <script  src="/js/underscore.js"></script>
   <script  src="/js/find_persons.js"></script>
   <script  src="/js/select_options.js"></script>
   <script  src="/js/find_property.js"></script>
   <script  src="/js/browsers_utils.js"></script>


   

   
   

<!--     <script type="text/javascript" src="js/profes.json"></script>-->
    

<script >
    var tipusreg = <?php echo "'".$tipusreg. "' \n";?>
    var horaris = <?php echo "'".$horaris. "' \n";?>
    horaris  =  JSON.parse(horaris)
    
    var profes=<?php  echo "'".$profes."'" ?>; 
    profes  =  JSON.parse(profes)
    var alumnes=<?php  echo "'".$alumnes."'" ?>; 
    alumnes  =  JSON.parse(alumnes)
         
   function set_color(elem, color, note){
       elem.style.backgroundColor = color;
       elem.note = note;
   }
    
    function set_note(elem, status) {
        elem.note = status;
    }
    
    function change_note(elem) {
    if(elem.note == undefined || elem.note == "unselected") {
        elem.note ="selected"
    }
    else {
         elem.note = "unselected"
    }
   }
    
    function change_color(elem) {      
    if(elem.note == undefined || elem.note == "unselected") {
        set_color(elem, selcolor, "selected")
    }
    else {
        set_color(elem, unselc, "unselected")
    }
   }
       
    function addtoray(array, string) {
        var len = array.length
        array[len]=string;
    }
    
    function day_seltimes(day, array) {
        //day es un elem on la meitat dels childones son undefined i la altra meitat
        //defined
        var len = day.childNodes.length
        len = (len -1)/2
        for(var i=0; i<len;++i) {
            var elem = day.childNodes[i*2+1]
            if(elem.note=="selected" && elem.className != "grid-header") {
                addtoray(array,elem.innerHTML)
            }
        }
    }
    
     
     
    function escriu_selec() { 
        day =[];
      for(var i=0;i<7;++i) {
          day[i] = [];
          day_seltimes(dies[i], day[i])
      }
    }
    
    function escriu_disp() {//writing showed horari to string variable
        escriu_selec();
        var hori = document.getElementById("horari");
        hori.value = day[0].toString() + ";" + day[1].toString() + ";" + 
                day[2].toString() + ";" + day[3].toString() + ";" +
                day[4].toString() + ";" + day[5].toString() + ";" +
                day[6].toString();
        for(var i=0;i<7;++i) {
        
      }
        
    }
    
    //now I have to write the inverse function of escriu_disp  
    
    function str_to_2dmatrix(string) { //string separat per , y ;
        var columns = [];
        var row = string.split(";");
        for(var i=0; i< row.length; ++i) {
            if( row[i] == "") columns[i] = [];
            else columns[i] = row[i].split(",");
    
        }
        return columns;
    }
    
    function troba(elem, array) {
        var ind = -1;
        for(var i=0; i<array.length; ++i) {
            if(array[i] == elem) {
                ind = i;
                break;
            }
            
        }
        return ind;
    }
    
    function dibuixa_dia(dia, column) {//draws horari from string
        //dia is an array of disponabilities
        //column is an html element containing more elements
        
        //day es un elem on la meitat dels childones son undefined i la altra meitat
        //defined
        var len = column.childNodes.length
        len = (len -1)/2
        for(var i=0; i<len;++i) {
            var elem = column.childNodes[i*2+1]
            if( troba(elem.innerHTML, dia)> -1 ) {
                set_color(elem, selcolor, "selected")                
            }
            
        }
        
        
    }
    
    function dibuixa_disp() {
        var day = str_to_2dmatrix(horari.value);
        
        for(var i=0; i<day.length; ++i) dibuixa_dia(day[i], dies[i]);
        
    }
    
    function canvia_totel(header, selec) {
        elem = header.parentNode;
        var len = elem.childNodes.length
        len = (len -1)/2
        for(var i=1; i<len;++i) {
            var item = elem.childNodes[i*2+1]
            if(selec) set_color(item, selcolor, "selected")
            else set_color(item, unselc, "unselected")
        }
    }
    
    function canvia_tot(elem) {
       
        change_note(elem);
        canvia_totel(elem, elem.note == "selected");
        
      //means index is i.
      
    }

    function upd_acro() {
        acronim.value = select.value;
        upd_nom();
    }

    function enable_canvi_horari() {
        tipureg.value="modifica";
        form.action="/registrar/disponibilitat/dispo_horaria.php"
        form.submit();
    }
    
    function cancela_canvi() {
        tipureg.value="veure";
        form.action="/registrar/disponibilitat/dispo_horaria.php"
        form.submit();
    }
         
         
</script>

    </head>
    
    <body onload="" >
        <div  id="tornar" class="w3-bar w3-teal w3-top" onclick="history_back()">
            <a href="" class="w3-bar-item w3-button">Torna enrere</a>
            
        </div>
        <br>
        <br>
        <h3 class="w3-center w3-text-black">
                  <i class="fa fa-home w3-margin-right" ></i>
                  Veient la disponibilitat de <span id='persona_canvi'></span> <b>NEM</b></h3>
        
        <form id="reloadform" action="/registrar/disponibilitat/dispo_horaria.php" method="post">
            <input name="tipureg" id="tipureg2" type="text" 
                                                           value="">
        </form>
        
        <form id="regForm" action="/php/registres/registra_dispon.php" method="post">
        <p >Franges de 30 minuts. <span class="disponible"> Disponible</span> 
            <span class="indisponible"> No disponible</span>
            <span> Persona: <input style="display:none" name="acronim" id="acronim" placeholder="acronim" type="text" 
                                   required readonly> 
                <select class="w3-select w3-button w3-teal" onchange="upd_acro()" style="display:none" 
                        id="acrooptions" name="acrooptions" ></select>           
                <input name="nom" id="nom" placeholder="nom" type="text" required readonly>
           <p> Clica a un dia "Dimecres" per posar 
                tot el dia disponible, clica a una hora, per posar-la disponible. 
                Tria la data d'inici i fi de l'horari.</p></span>
            <span style="display:none" > Horari: <input name="horari" id="horari" placeholder="horari" 
                                                        type="text" ></span>
            <span style="display:none"> tipus: <input name="tipureg" id="tipureg" type="text" 
                                                           value=<?php echo "'$tipusreg'" ?>></span>
        </p>
        <div>
        <p>Data inici:<input id="data_inici" placeholder="data inici..."  
                           oninput="this.className = ''" name="data_inici" 
               type="date" required>  </p>
        <p>Data fi: <input id="data_caduca" placeholder="data caducitat..."  oninput="this.className = ''" name="data_caduca" 
                               type="date" required></p>  <p>de la disponibilitat. </p>
         </div>
        
                <button type="submit" id="guarda" class = "w3-button w3-teal top-rounded-1" 
                        onclick="escriu_disp()">Guarda disponibilitat</button>
                <button type="submit" id="elimina" class = "w3-button w3-teal top-rounded-1" 
                        onclick="tipureg.value='elimina'">Elimina disponibilitat</button>
        <div  id="modifica" class = "w3-button w3-teal top-rounded-1" 
                        onclick="enable_canvi_horari();">Modifica disponibilitat</div>
             
        <div  id="cancela" class="w3-bar-item w3-button lightest top-rounded-1" onclick="cancela_canvi();">
           Cancela
            
        </div>
        
            </form>
        
   
  <div class="grid-container" id="grid_horari">
      
      
 <div class="grid-column">
  <div class="grid-header" >Dilluns</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
 <div class="grid-column">
  <div class="grid-header">Dimarts</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
    <div class="grid-column">
  <div class="grid-header">Dimecres</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>

      <div class="grid-column">
  <div class="grid-header">Dijous</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
      <div class="grid-column">
  <div class="grid-header">Divendres</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
      
      <div class="grid-column">
  <div class="grid-header">Dissabte</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
      
      <div class="grid-column">
  <div class="grid-header">Diumenge</div>
  <div class="grid-item">7:00</div>
  <div class="grid-item">7:30</div>
  <div class="grid-item">8:00</div>
  <div class="grid-item">8:30</div>
  <div class="grid-item">9:00</div>  
  <div class="grid-item">9:30</div>
  <div class="grid-item">10:00</div>
  <div class="grid-item">10:30</div>  
  <div class="grid-item">11:00</div>
  <div class="grid-item">11:30</div>
  <div class="grid-item">12:00</div> 
  <div class="grid-item">12:30</div> 
  <div class="grid-item">13:00</div>  
  <div class="grid-item">13:30</div>
  <div class="grid-item">14:00</div>
  <div class="grid-item">14:30</div> 
  <div class="grid-item">15:00</div>
  <div class="grid-item">15:30</div>
  <div class="grid-item">16:00</div>  
  <div class="grid-item">16:30</div>  
  <div class="grid-item">17:00</div>
  <div class="grid-item">17:30</div>
  <div class="grid-item">18:00</div> 
  <div class="grid-item">18:30</div> 
  <div class="grid-item">19:00</div>
  <div class="grid-item">19:30</div>
  <div class="grid-item">20:00</div>  
  <div class="grid-item">20:30</div>  
  <div class="grid-item">21:00</div>
  <div class="grid-item">21:30</div>
  <div class="grid-item">22:00</div> 
  <div class="grid-item">22:30</div>
  <div class="grid-item">23:00</div>
  <div class="grid-item">23:30</div> 
</div>
      
      
      
      
</div>


        
    </body>
    <script>
      
        
        
            var elem =0;
            var selcolor = "#e7ffe7"
            var unselc = "#e299c3" 
            var day =[];
            var dies = document.getElementsByClassName("grid-column");
            
            
            var acronim = document.getElementById("acronim");
            var nom = document.getElementById("nom");
            var horari = document.getElementById("horari");
            var tipureg = document.getElementById("tipureg");
            var data_inici = document.getElementById("data_inici");
            var data_caduca = document.getElementById("data_caduca");
            var select = document.getElementById("acrooptions");
            var taula = document.getElementById("grid_horari");
            var guarda = document.getElementById("guarda");
            var elimina = document.getElementById("elimina");
            var cancela = document.getElementById("cancela");
            
            var pers_canv = document.getElementById("persona_canvi");
            
            var tornar = document.getElementById("tornar");

            var header = document.getElementsByClassName("grid-header");
            var collection = document.getElementsByClassName("grid-item");
            var form = document.getElementById("regForm"); 

            var form2 = document.getElementById("reloadform"); 
            form2.style.display="none";
            var tipureg2 = document.getElementById("tipureg2");
            var modifica = document.getElementById("modifica");
            modifica.style.display="none";
             
            var capat=<?php  echo "'".$capat."'" ?>;
            
            elimina.style.display="none";
            
            <?php
            if($tipusreg=="modifica") {
                echo "acronim.value = '" . $_POST['acronim'] . "' \n";
                echo "horari.value = '" . $_POST['horari']. "'\n";
                echo "data_inici.value = '" . $_POST['data_inici']. "'\n";
                echo "data_caduca.value = '" . $_POST['data_caduca']. "'\n";
                
            }
            ?>
            
            
            if(tipusreg=="veure") {
                 
                 <?php
                if($tipusreg=="veure") {
                echo "acronim.value = '" . $_POST['acronim'] . "' \n";
                }
                ?>
                
                tornar.style.display = "inline"
                cancela.style.display = "none"
                        
                var elem = find_property(acronim.value, horaris, "acronim");
                horari.value=elem.horari
                data_inici.value =elem.data_inici
                data_caduca.value = elem.data_caducitat
                
                acronim.readOnly = true; 
                nom.readOnly = true; 
                horari.readOnly = true; 
                data_inici.readOnly = true; 
                data_caduca.readOnly = true; 
                taula.disabled = true; 
                guarda.style.display="none";
                
                if(capat == "no") modifica.style.display="inline";
                dibuixa_disp();
                
            } else {
            
               
                 

                for (var i = 0; i < header.length; i++) {
                    var elem = header[i];
                    (function(x){//doing this to avoid "i" changing value after loop
                     //works like a charm
                     elem.addEventListener("click", function(){ canvia_tot(header[x]); }); 
                    
                })(i)
                   
                  }

                for (var i = 0; i < collection.length; i++) {
                    var elem = collection[i];
                    (function(x){//doing this to avoid "i" changing value after loop
                     //works like a charm
                     elem.addEventListener("click", function(){ change_color(collection[x]); }); 
                    
                })(i)
                  } 
                  
                if(tipusreg=="modifica") {
                    elimina.style.display="inline";
                    dibuixa_disp();
                } else if(tipusreg=="nou") {

                //putting all acronims into an array
                var acronims = alumnes.map(function(a) {return a.acronim});//makes array of the property
                var acro2 = profes.map(function(a) {return a.acronim});//makes array of the property
                acronims = acronims.concat(acro2);

                var names = alumnes.map(function(a) {return a.nom});//makes array of the property
                var names2 = profes.map(function(a) {return a.nom});//makes array of the property
                names = names.concat(names2);

                crea_options(select.id, names, acronims)
                select.style.display="inline";
                nom.style.display = "none"
                upd_acro();
                }
            }
            
            function upd_nom() {
                nomac = findname(acronim.value, profes, alumnes)
                if(nomac != false) {
                    nom.value = nomac
                    pers_canv.innerHTML = nomac;
                }
            }
            
            upd_nom()
            
            
        </script>
         
        
        <script>
        
        
        </script>
        
    
</html>
