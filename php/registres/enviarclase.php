<?php
   ...
   //  http://localhost/PHPexec.php?f=C:\Roma\Nebeans_repsol\Helico\public_html\php\sendscore.php
    ...
    include("../getprofes.php");//contains functions for profes and students   

    //  vars per una nova clase

        $tipusreg = $_POST['tipureg'];

       if($tipusreg=="elimina" || $tipusreg=="modifica") $id_clase = $_POST['id_clase'];
        $dia_clase = $_POST['dia'];
        $hores_de_clase = $_POST['temps'];
        $profe = $_POST['nomprofes'];
        $alumne = $_POST['nomalumnes'];
        //be cafeful wiht 'comentari'. Si el comentari té una coma ', donarà
        //error.
        $profe = mysqli_real_escape_string($conn, $profe);
        $alumne = mysqli_real_escape_string($conn, $alumne);
        $cafe = $_POST['cafe'];
        $desplaç = $_POST['tdesp'];
        $comentari = $_POST['comentari'];
        //be cafeful wiht 'comentari'. Si el comentari té una coma ', donarà
        //error.
        $comentari = mysqli_real_escape_string($conn, $comentari);
      
      if($tipusreg=="modifica") {
          
           $cafe = $_POST['cafe'];
           $desp = $_POST['tdesp'];
           $pagada_by_client = $_POST['pagada_by_client'];
           $pagada_to_profe = $_POST['pagada_to_profe'];
                  
           $original_dia = $_POST['original_dia'];
           $original_temps = $_POST['original_temps'];
           $original_profe = $_POST['original_profe'];
           $original_comentari = $_POST['original_comentari'];
           $original_alumne = $_POST['original_alumne'];
           $original_desplaç = $_POST['original_desplaç'];
           $original_cafe = $_POST['original_cafe'];
           
            //be cafeful wiht 'comentari'. Si el comentari té una coma ', donarà
            //error.
            $original_comentari = mysqli_real_escape_string($conn, $original_comentari);
            $original_profe = mysqli_real_escape_string($conn, $original_profe);
            $original_alumne = mysqli_real_escape_string($conn, $original_alumne);
      }
...
$anom = getname("alumnes", $alumne);
$pnom = getname("profes", $profe);   


$anomphp = json_decode($anom);
$pnomphp = json_decode($pnom);

$failed=false;
if($tipusreg=="elimina" && isset($_POST['id_clase'])) {
    
   
    
    $presql = "FROM registre_clases WHERE "
            . "id_clase='" . $id_clase . "' AND "
            . "dia='" . $dia_clase . "' AND "
            . "nom_profe = '" . $profe . "' AND "
            . "nom_alumne = '" . $alumne . "' AND "
            . "cafe = " . $cafe . " AND "
            . "temps = '" . $hores_de_clase . "'";
    $sql = "SELECT * " . $presql;
    ...
    if( count($json) > 1) { //checking no more than 1 data row is gonna be
        //deleted
        $failmsg="error: More than 1 clase with these parameters! Query: " . $sql. " <br>";
        $failed=true;
        exit($failmsg);
    ...
    $sql = "DELETE " . $presql;
     $successmessage="<br>Linia eliminada de les clases correctament";
    }
} elseif($tipusreg=="modifica") {
     $where = " WHERE "
            . "id_clase='" . $id_clase . "' AND "
            . "dia='" . $original_dia . "' AND "
            . "nom_profe = '" . $original_profe . "' AND "
            . "nom_alumne = '" . $original_alumne . "' AND "
            . "comentari = '" . $original_comentari . "' AND "
            . "desplaçament = '" . $original_desplaç . "' AND "
            . "cafe = " . $original_cafe . " AND "             
            . "temps = '" . $original_temps . "'";
     
     // comprovant que hi ha alguna classe amb aquests paràmetres:
     $sql = "SELECT * FROM registre_clases " . $where;     
     $result=mysqli_query($conn,$sql );
    $json = array();
    while($row = mysqli_fetch_assoc($result))     
     {
    ...
    } elseif(count($json) == 0) {//checking that at least 1 data row is selected
        $failmsg="error: Cap clase with these parameters! Query: " . $sql. " <br>";
        $failed=true;
        exit($failmsg);        
    } else {
     $set=  "SET dia='" . $dia_clase . "', "
            . "nom_profe = '" . $profe . "', "
            . "nom_alumne = '" . $alumne . "', "
            . "comentari = '" . $comentari . "', "
            . "desplaçament = '" . $desp . "', "
            . "cafe = " . $cafe . ", "
            . "pagada_by_client = " . $pagada_by_client . ", "
            . "pagada_to_profe = " . $pagada_to_profe . ", "
            . "temps = '" . $hores_de_clase . "'";
     
     $sql = "UPDATE registre_clases " . $set . $where;
     $successmessage="<br>Clase modificada correctament!";
     $failmsg="<br>Problema modificant la classe.";
    }
    ...
    $where = " WHERE "            
            ...
            . "nom_profe = '" . $profe . "' AND "
            . "nom_alumne = '" . $alumne . "' AND "            
            . "comentari = '" . $comentari . "' AND "
            . "desplaçament = '" . $desplaç . "' AND "
            . "cafe = " . $cafe . " AND "             
            . "temps = '" . $hores_de_clase . "'";
...
//    echo "<br><br> result: " . $result;
    ...
    if( count($json) > 0) {
        //modified
        $failmsg="error: Ja hi ha una classe amb aquests paràmetres! Query: "; 
//                . $sql. " <br>";
        $failed=true;
        exit($failmsg);
    } else {   // ara sí, afegeix la classe sino hi ha ja una classe així:
        $sql = "INSERT INTO registre_clases (dia, temps, 
            nom_profe, nom_alumne, cafe, desplaçament, comentari)
       VALUES ('" . $dia_clase . "', '" . $hores_de_clase .
                "', '" . $profe . "', '" . $alumne . "', " . $cafe .
                ", '" . $desplaç ."', '" . $comentari ."')";

        $failmsg="<br>Error afegint linia a reserves: Query: " . $sql . " " ;
        $successmessage="<br>Linia afegida a reserves correctament";
    }
} elseif(! isset($_POST['id_clase'])) {
    $failmsg="<br>Error: Provant d'eliminar o modificar una clase però id_clase no s'ha donat." ;
    $failed=true;
    exit($failmsg);
}
...
<html>
   ...
   <link rel="stylesheet" href="/css/slideformw3.css">
    ...
    <script>
        var profe = <?php echo $pnom?>;
        var alumne = <?php echo $anom?>;
    ...
    <body onload="">
        ...
        <div class="w3-bar w3-teal w3-top">
            <a href="/registrar/registra_clases.php" class="w3-bar-item w3-button">Regsitra una altra clase</a>
        </div>
        <br>
        <br>
       <h3 class="w3-center w3-text-black">
                  <i class="fa fa-home w3-margin-right" ></i>
                  Clase registrada <b>NEM</b></h3>

        


<form id="regForm" action="" method="post" >
  <h1>Detalls de la clase:</h1>
  <!-- One "tab" for each step in the form: -->
 
  
  <p>  <label for="cars">Profe:</label> 
           <select readonly name="nomprofes" id="nomprofes" onclick="deleteinvalid(this)">
               <option value="<?php echo $profe ?>"><?php echo $pnomphp[0] -> nom; ?></option>

  </select></p>
  <p>  <label for="cars">Alumne:</label> 
           <select readonly name="nomalumnes" id="nomalumnes" onclick="deleteinvalid(this)">
               <option value="<?php echo $alumne ?>"><?php echo $anomphp[0] -> nom; ?></option>

  </select></p>
  
  Dia:
       <div id="wrapper">
    <p><input readonly id="dia" placeholder="naixement..."  oninput="this.className = ''" name="dia" type="date" 
               value="<?php echo $dia_clase ?>"></p>
        </div>
       

       <p>  <label for="cars">Hores fetes (min):</label> 
           <select  name="temps" id="temps" >
               <option value="<?php echo $hores_de_clase ?>"><?php echo $hores_de_clase ?></option>

  </select></p>
  
  <p>  <label>Temps de desplaçament fins a l'alumn@ (min):</label> 
           <select name="tdesp" id="tdesp" readonly>
               <option value="<?php echo $desplaç ?>"><?php echo $desplaç ?></option>

  </select></p>
  
  <p>  <label>Clase feta en un cafè/restaurant:</label> 
           <select name="cafe" id="cafe" readonly>
              <option value="<?php echo $cafe ?>"><?php if($cafe) echo "Sí"; else echo "No"; ?></option>

  </select></p>
Comentari:
       <div id="wrapper">
    <p><input readonly  placeholder="naixement..."  oninput="this.className = ''"  type="text" 
               value="<?php echo stripcslashes($comentari) ?>"></p>
        </div>
