<?php
   ...
   //  http://localhost/PHPexec.php?f=C:\Roma\Nebeans_repsol\Helico\public_html\php\sendscore.php
    ...
    //  vars per una nova clase
        ...
        $parentesc = $_POST['parentesc'];
        $comentari = $_POST['comentari'];
        $alumne = $_POST['alumne'];
        $datainici = $_POST['data_inici'];
        $data_fi = $_POST['data_fi'];
        if($data_fi == "") $data_fi = "0001-01-01"; //aixo vol dir que no hi ha 
                                                //data de fi fixada
        
        if($materia == "") {
              $pagador = $_POST['pagador'];
              $pagnom = getname("pagadors", $pagador);  
              $pagnomphp = json_decode($pagnom);
        }
        else {
              $profe = $_POST['profe'];              
...
echo "<br>";
       ...
       if($materia != "") {
           //si hi ha materi, vol dir que es registra a un profe
            $sql = "INSERT INTO aprofe_conexions (profe, alumne, materia, comentari, data_inici,
                data_fi)
                    VALUES ('" . $profe . "', '" . $alumne . "', '" . $materia .
                    "', '" . $comentari . "', '" . $datainici . 
                     "', '" . $data_fi ."')";
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                echo "<br>Linia afegida a conexions alumne-profe correctament";
              } else {
                  $error = "error";
               exit("<br>Error afegint linia a connexions alumne-profe: " . $conn->error);
              }

       }
       else {
           // vés i registra el pagador
            $sql = "INSERT INTO apagador_conexions (pagador, alumne, parentesc, comentari)
                    VALUES ('" . $pagador . "', '" . $alumne . "', '" . $parentesc .
                    "', '" . $comentari . "')";

           
            if ($conn->query($sql) === TRUE) {
        
                echo "<br>Linia afegida a conexions alumne-pagador correctament";
              } else {
                  $error = "error";
               exit("<br>Error afegint linia a connexions alumne-pagador: " . $conn->error);
              }
       }
...
<html>
   ...
   <link rel="stylesheet" href="/css/slideformw3.css">
    ...
    var esprofe = <?php if($materia != "") echo "true";
    else echo "false";?>
    
    var datafi = "<?php if($materia != "")  echo $data_fi  ?>";
...
<!--     <script type="text/javascript" src="js/profes.json"></script>-->
    ...
    <body onload="">
        ...
        <div class="w3-bar w3-teal w3-top">
            <a href="/registrar/registra_connexio.php" class="w3-bar-item w3-button">Registra una altra connexió</a>
        </div>
        <br>
        <br>
       <h3 class="w3-center w3-text-black">
                  <i class="fa fa-home w3-margin-right" ></i>
                  Connexió registrada <b>NEM</b></h3>

        


<form id="regForm" action="" method="post" >
   <h1>Detalls de la connexió alumne - <?php if($materia != "") echo "profe";
    else echo "pagador";?></h1>
  ...
  <!-- One "tab" for each step in the form: -->
    ...
    <p>      <label>nom de l'alumne:</label>
    <select  id="nomalumnes" placeholder="Juan Soraya..."  oninput="this.className = ''"
            name="alumne" type="text" readonly >
     ...
     <p>
    <div id="profediv" >
    <label>nom del profe</label>
    <select  id="nomprofes" placeholder="Maria de cases..."  
             oninput="this.className = ''" name="profe" type="text" readonly>
       <option value="<?php if($materia != "") echo $profe ?>">
           <?php if($materia != "")  echo $pnomphp[0] -> nom; ?></option>
    ...
    <select  id="nompagadors" placeholder="Juana de Arc..."  
               oninput="this.className = ''" name="pagador" type="text" readonly>
         <option value="<?php if($materia == "")  echo $pagador ?>">
             <?php if($materia == "")  echo $pagnomphp[0] -> nom ?></option>
    </select></div>    
   ...
   <p>  
     <div id="data_inici_div" >
     <label for="cars">Data inici del mestratge</label> 
        <input  id="data_inici" placeholder="data inici..."  oninput="this.className = ''" name="data_inici" 
               type="date" readonly value="<?php if($materia != "") echo $datainici; ?>">
    
    </div>
      </p>
      
   <p>
     <div id="data_fi_div" class="<?php if($materia != "" && $data_fi == "0001-01-01")  echo "palid"  ?>" >
     <label for="cars">Data fi del mestratge</label> 
        <input  id="data_fi" placeholder="data fi..."  oninput="this.className = ''" name="data_fi" 
               type="date" value="<?php if($materia != "")  echo $data_fi  ?>" readonly>
   ...
   <p>
    <div id="materiadiv" >
     <label>Materia:</label><input  id="materia" placeholder="matemàtiques"  
               oninput="this.className = ''" name="materia" type="text" readonly 
   ...
   <p>
    <div id="parentescdiv" >
     <label>Parentesc:</label>
      <input  id="parentesc" placeholder="pare"  
              oninput="this.className = ''" name="parentesc" type="text" readonly
              value="<?php echo $parentesc ?>">
  ...
  <p>
    <div id="comentaridiv" >
        ...
        <input  id="comentari" placeholder="vol classes de fraccions especialment..."  
                name="comentari" value="<?php echo $comentari?>" type="text" readonly>
    </div></p>    
    ...
    <script>
     ...
     profediv = document.getElementById("profediv");
     pagadiv = document.getElementById("pagadiv");
     materiadiv = document.getElementById("materiadiv");
     parentescdiv = document.getElementById("parentescdiv");
     var datainicidiv = document.getElementById("data_inici_div");
     var datafidiv = document.getElementById("data_fi_div");
     
     if(esprofe) {
         // hide payer info
         pagadiv.style.display = "none";
         parentescdiv.style.display = "none";
         
     } else {
         // hide profe info
         profediv.style.display = "none";
         materiadiv.style.display = "none";
         datainicidiv.style.display = "none";
         datafidiv.style.display = "none";
     }
    </script>
