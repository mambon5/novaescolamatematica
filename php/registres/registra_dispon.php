<?php
   ...
   //  http://localhost/PHPexec.php?f=C:\Roma\Nebeans_repsol\Helico\public_html\php\sendscore.php
    ...
    include("../get_elems.php");
       
        $tipureg = $_POST['tipureg']; //tipus de registre (modificar, nou, cancelar)
    //  vars per una nova clase
        $data_inici = $_POST['data_inici'];
        $data_caduca = $_POST['data_caduca'];
        $acronim = $_POST['acronim'] ;
        $horari = $_POST['horari'];
        
        $nom_taula="disponibilitat";
...
echo "<br>";
 ...
 $exitmsg = "";
 $failmsg = "";
 //get possible current regsitries of this set
    $sql = "SELECT data_inici, data_caducitat,
         acronim, horari FROM " . $nom_taula . " WHERE acronim = '" . $acronim . "'";
...
if ($tipureg=="modifica" ) {
     //si volen modificar però no hi ha registre, afegim un registre
     if( count($json) == 0) {
               $tipureg = "nou";
           }
    else {         
            $where = " WHERE "
                    . " data_inici= '" . $data_inici . "' AND "
                    . "data_caducitat = '" . $data_caduca . "' AND "
                    . "acronim =  '" . $acronim . "'";

           $sql = "UPDATE  " . $nom_taula . "  SET horari='" . $horari . "' " . $where;
           $exitmsg= "<br>Linia modificada a ".$nom_taula." correctament";
           $failmsg="<br>Error modificant linia a ".$nom_taula." : ";
    }
 } 
 if($tipureg=="nou") {
      if(!acronim($acronim)) {
          $failmsg="error: el nom " . $acronim . 
            "no és un acrònim valid";
        $failed=true;
        exit($failmsg);
      } else {     
            //check there is no dispo already

           if( count($json) > 0) {
               $failmsg="error: disponibilitat for " . $acronim . 
                   "already exists! (".count($json)."). <br> "
                       . "Try modifying it here <a href=\"/modifica/modifica_dispon.php\">"
                       . "Modifica disponibilitats</a>"  . $conn->error;
               $failed=true;
               exit($failmsg);
           } else {

               $sql = "INSERT INTO " . $nom_taula . " (data_inici, data_caducitat,
                   acronim, horari)
              VALUES ('" . $data_inici . "', '" . $data_caduca .
                       "', '" . $acronim . "', '" . $horari . "')";
               $exitmsg= "<br>Linia afegida a ".$nom_taula." correctament per a " . $acronim;
               $failmsg="<br>Error afegint linia a ".$nom_taula." : ";
           }
      }
 }  elseif ($tipureg=="elimina" ) {
      if( count($json) == 1) {
          $sql = "DELETE FROM " . $nom_taula . " WHERE acronim = '" . $acronim . "'";     
          $exitmsg = "<br> disponibilitat for " . $acronim .
                " deleted with success. ";
    } elseif(count($json) > 1) {
        $failmsg = "error: multiples disponibilitats for " . $acronim .
                ". We don't know which one to delete. Delete manually.";
        $failed=true;
        exit($failmsg);
    }
    else {
        $failmsg="error: disponibilitat for " . $acronim . 
            "doesn't exist! (".count($json)."). <br> "
                . "Try entering a new one here <a href=\"/registrar/disponibilitat/dispo_horaria.php\">"
                . "Registrar disponibilitats</a>"  . $conn->error;
        $failed=true;
        exit($failmsg);
    }
 }
 
    if (!$failed && $conn->query($sql) === TRUE) {//execute query
        
        echo  $exitmsg;
      } else {// don't execute query
          $error = "error";
...
<html>
    <head>
       <link rel="icon" type="image/x-icon" href="/w3images/favicon.png">
        ...
        <head>
    <title>Veure disponibilitats</title>
   <link rel="stylesheet" href="/css/slideformw3.css">
    ...
    <script src="/js/list_json.js"></script>
    <script src="/js/print_table.js"></script>
...
<!--     <script type="text/javascript" src="js/profes.json"></script>-->
    ...
    <script>
      ...
      var horaris=<?php  echo "'".$horaris."'" ?>; 
      horaris  =  JSON.parse(horaris)
      //falta separar
      
      function canvia_dispo(item) {
            data_inici.value = item.childNodes[0].innerHTML;
            data_caduca.value = item.childNodes[1].innerHTML;
            acronim.value = item.childNodes[2].innerHTML;
            horari.value = item.childNodes[3].innerHTML;
            tipureg.value = "modifica";      
...
//        json_to_select(valclaus,"nom","clau","valclaus")
    ...
    <body onload="">
        ...
        <div class="w3-bar w3-teal w3-top" >
            <a href="/" class="w3-bar-item w3-button">Torna enrere</a>
        </div>
      
    </body>    
    
    <script>
        var columns = Object.getOwnPropertyNames(horaris[0]);
       print_table(horaris, columns, "taula")
    </script>
    ...
    <script>
       ...
       var items = document.getElementsByClassName("tabitem");
            for (let i = 0; i < items.length; i++) {
                 elem = items[i];
                elem.addEventListener("click", function(){ canvia_dispo(items[i]); }); 
              }       
              
        var acronim = document.getElementById("acronim");
        var horari = document.getElementById("horari");
        var tipureg = document.getElementById("tipureg");
        var data_inici = document.getElementById("data_inici");
        var data_caduca = document.getElementById("data_caduca");
